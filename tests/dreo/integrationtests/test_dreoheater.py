"""Integration Tests for Dreo Heaters"""
...
class TestDreoHeater(IntegrationTestBase):
    """Test Dreo Heater class and PyDreo together."""
    ...
    def test_HSH009S(self):  # pylint: disable=invalid-name
        ...
        with patch(PATCH_SCHEDULE_UPDATE_HA_STATE):
            ...
            self.verify_expected_entities(numbers, ["Target Temperature"])

            sensors = sensor.get_entries([pydreo_heater])
            self.verify_expected_entities(sensors, ["Status"])

            assert(HEATER_MODE_COOLAIR in self.get_entity_by_key(sensors, "Status").entity_description.options)